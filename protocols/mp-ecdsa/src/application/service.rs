...
pub trait ProtocolFromConfig {
    fn name() -> &'static str;
    fn generate() -> Pin<Box<dyn SendFuture<'static, ()>>>;
}
...
pub fn new_full<T: ProtocolFromConfig>(
    ...
    if role.is_authority() {
        tangle_primitives::utils::insert_controller_account_keys_into_keystore(
            &config,
            Some(keystore_container.keystore()),
        );

        tangle_primitives::utils::insert_dkg_account_keys_into_keystore(
            &config,
            Some(keystore_container.keystore()),
        );
