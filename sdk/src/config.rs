...
impl<RwLock: lock_api::RawRwLock> GadgetConfiguration<RwLock> {
    ...
    pub async fn client(&self) -> Result<crate::clients::tangle::runtime::TangleClient, Error> {
        let ws_endpoint = &self.ws_rpc_endpoint;
        let http_endpoint = &self.http_rpc_endpoint;
        ...
        let resolved_client = futures::future::select_ok([
            Box::pin(ws_task) as Pin<Box<dyn Future<Output = _>>>,
            Box::pin(http_task),
        ])
        .await
        .map_err(|err| Error::UnsupportedProtocol(err.to_string()))?
        .0;
