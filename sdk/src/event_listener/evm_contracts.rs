...
pub struct EthereumHandlerWrapper<W: EvmEventHandler> {
    handler: Arc<W>,
    contract: W::Contract,
    chain_id: u64,
...
pub type EvmWatcherWrapperContext<W> = (<W as EvmEventHandler>::Contract, Arc<W>);

#[async_trait::async_trait]
impl<Watcher: EvmEventHandler>
    EventListener<Vec<(Watcher::Event, alloy_rpc_types::Log)>, EvmWatcherWrapperContext<Watcher>>
    for EthereumHandlerWrapper<Watcher>
{
    ...
    async fn next_event(&mut self) -> Option<Vec<(Watcher::Event, alloy_rpc_types::Log)>> {
        ...
        info!("Querying events for filter, address: {}, from_block: {}, to_block: {}, event_signature: {}", contract.address(), block + 1, dest_block, Watcher::Event::SIGNATURE_HASH);
...
impl<Watcher: EvmEventHandler> EthereumHandlerWrapper<Watcher> {
