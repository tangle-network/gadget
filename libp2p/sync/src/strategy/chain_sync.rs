        ...
        + 'static,
{
    ...
    pub fn add_peer(&mut self, peer_id: PeerId, best_hash: B::Hash, best_number: NumberFor<B>) {
        match self.add_peer_inner(peer_id, best_hash, best_number) {
            Ok(Some(request)) => self
                .actions
                .push(ChainSyncAction::SendBlockRequest { peer_id, request }),
            Ok(None) => {}
    ...
    fn on_block_data(
        ...
        let new_blocks: Vec<IncomingBlock<B>> = if let Some(peer) = self.peers.get_mut(peer_id) {
            ...
            if request
                .as_ref()
                .map_or(false, |r| r.direction == Direction::Descending)
            {
    ...
    fn on_block_justification(
        ...
        if let PeerSyncState::DownloadingJustification(hash) = peer.state {
            ...
            if let Some((peer_id, hash, number, justifications)) = self
                .extra_justifications
                .on_response(peer_id, justification)
            {
    ...
    pub fn on_validated_block_announce(
        ...
        let parent_status = self
            .block_status(announce.header.parent_hash())
    ...
    fn validate_and_queue_blocks(&mut self, mut new_blocks: Vec<IncomingBlock<B>>, gap: bool) {
        ...
        self.actions.push(ChainSyncAction::ImportBlocks {
            origin,
            blocks: new_blocks,
        })
    }
    ...
    pub fn on_block_response(
        ...
        let block_response = BlockResponse::<B> {
            id: request.id,
            blocks,
        };
    ...
    fn block_requests(&mut self) -> Vec<(PeerId, BlockRequest<B>)> {
        ...
        let max_parallel = if is_major_syncing {
            1
        } else {
        ...
        for (result, hash) in results {
            ...
            match result {
                ...
                Ok(BlockImportStatus::ImportedUnknown(number, aux, peer_id)) => {
                    ...
                    let state_sync_complete = self
                        .state_sync
                        .as_ref()
                        .map_or(false, |s| s.target_hash() == hash);
                ...
                Err(BlockImportError::BadBlock(peer_id)) => {
                    if let Some(peer) = peer_id {
                        ...
                        self.actions
                            .push(ChainSyncAction::DropPeer(BadPeer(peer, rep::BAD_BLOCK)));
                    }
                }
...
pub fn validate_blocks<Block: BlockT>(
    ...
    if let Some(request) = request {
        ...
        let block_header = if request.direction == Direction::Descending {
            blocks.last()
        } else {
            blocks.first()
        }
