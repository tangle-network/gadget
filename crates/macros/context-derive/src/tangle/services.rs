...
pub fn generate_context_impl(
    ...
    let config_ty = quote! {
        ::blueprint_sdk::macros::ext::contexts::services::TangleServicesClient<::blueprint_sdk::macros::ext::tangle::tangle_subxt::subxt::PolkadotConfig>
    };

    quote! {
        #[::blueprint_sdk::macros::ext::async_trait::async_trait]
        impl #impl_generics ::blueprint_sdk::macros::ext::contexts::services::ServicesContext for #name #ty_generics #where_clause {
            fn get_call_id(&mut self) -> &mut Option<u64> {
                &mut #field_access_call_id
            }

            async fn services_client(&self) -> #config_ty {
                let rpc_client = ::blueprint_sdk::macros::ext::tangle::tangle_subxt::subxt::OnlineClient::from_url(
                    &#field_access_config.http_rpc_endpoint
                )
                .await
                .expect("Failed to create RPC client");

                ::blueprint_sdk::macros::ext::contexts::services::TangleServicesClient::new(rpc_client)
            }
        }
    }
}
