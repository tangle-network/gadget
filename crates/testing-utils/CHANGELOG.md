# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.1.0](https://github.com/tangle-network/gadget/releases/tag/gadget-testing-utils-v0.1.0) - 2025-03-10

### Added

- get blueprint protocol basic summation gossip working ([#678](https://github.com/tangle-network/gadget/pull/678))
- new networking ([#664](https://github.com/tangle-network/gadget/pull/664))
- Add multinode test executor
- *(cargo-tangle)* eigenlayer deployment ([#645](https://github.com/tangle-network/gadget/pull/645))
- testing improvement ([#629](https://github.com/tangle-network/gadget/pull/629))
- blueprint pre-registration ([#625](https://github.com/tangle-network/gadget/pull/625))
- gadget workspace migration

### Fixed

- finish migration of new job system ([#699](https://github.com/tangle-network/gadget/pull/699))
- *(anvil-test-utils)* stop using relative paths for default state ([#684](https://github.com/tangle-network/gadget/pull/684))
- cli run flow and eigenlayer versions ([#677](https://github.com/tangle-network/gadget/pull/677))
- update some tests
- *(tangle-test-utils)* stop duplicating operator preferences ([#671](https://github.com/tangle-network/gadget/pull/671))
- eigen test contract size error ([#673](https://github.com/tangle-network/gadget/pull/673))
- blueprint examples ([#643](https://github.com/tangle-network/gadget/pull/643))
- pre-registration test bug ([#644](https://github.com/tangle-network/gadget/pull/644))
- remove git dependencies for release ([#623](https://github.com/tangle-network/gadget/pull/623))

### Other

- remove old event listeners and runners
- bump alloy & eigensdk ([#696](https://github.com/tangle-network/gadget/pull/696))
- *(blueprint-serde)* update `tangle_subxt` ([#690](https://github.com/tangle-network/gadget/pull/690))
- Update to tangle main services ([#674](https://github.com/tangle-network/gadget/pull/674))
- cleanup crate features & update `tangle-subxt` ([#642](https://github.com/tangle-network/gadget/pull/642))
- add descriptions to crates ([#616](https://github.com/tangle-network/gadget/pull/616))
