...
use blueprint_job_router::{BoxError, JobCall, Router};
    ...
    F: Future<Output = ()> + Send + 'static,
{
    async fn run(self) -> Result<(), Error> {
        ...
        poll_fn(|ctx| router.poll_ready(ctx))
            .await
            .unwrap_or_else(|_err| {});

        let mut producer_stream = futures::stream::select_all(producers);
        let mut background_services = futures::future::select_all(background_futures);
