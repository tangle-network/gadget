    ...
    T: DeserializeOwned,
{
    ...
    fn into_future(self) -> Self::IntoFuture {
        ...
        Box::pin(async move {
            ...
            let filter = filter.unwrap_or_else(|| "blueprint=trace".parse().unwrap());
...
struct TestMakeWriter {
    write: Arc<std::sync::Mutex<Option<Vec<u8>>>>,
}

impl TestMakeWriter {
    fn new() -> (Self, Handle) {
        let write = Arc::new(std::sync::Mutex::new(Some(Vec::<u8>::new())));
