        ...
        + 'static,
{
    ...
    pub fn new(
        ...
        let warp_sync_target_block_header_rx_fused = warp_sync_target_block_header_rx
            .map_or(futures::future::pending().boxed().fuse(), |rx| {
                rx.boxed().fuse()
            });
    ...
    fn process_block_announce_validation_result(
        ...
        match validation_result {
            ...
            BlockAnnounceValidationResult::Process {
                is_new_best,
                peer_id,
                announce,
            } => {
                if let Some((best_hash, best_number)) =
                    self.strategy
                        .on_validated_block_announce(is_new_best, peer_id, &announce)
                {
                ...
                if let Some(data) = announce.data {
                    if !data.is_empty() {
                        self.block_announce_data_cache
                            .insert(announce.header.hash(), data);
                    }
                }
            }
            BlockAnnounceValidationResult::Failure {
                peer_id,
                disconnect,
            } => {
                if disconnect {
                ...
                self.network_service
                    .report_peer(peer_id, rep::BAD_BLOCK_ANNOUNCEMENT);
            }
        }
    }
        ...
        for (peer_id, ref mut peer) in self.peers.iter_mut() {
            ...
            if inserted {
                ...
                let message = BlockAnnounce {
                    header: header.clone(),
                    state: if is_best {
                        Some(BlockState::Best)
                    } else {
                        Some(BlockState::Normal)
                    },
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            self.is_major_syncing
                .store(self.strategy.is_major_syncing(), Ordering::Relaxed);
    ...
    fn perform_periodic_actions(&mut self) {
        ...
        if self.last_notification_io.elapsed() > INACTIVITY_EVICT_THRESHOLD {
            ...
            for peer in self.peers.keys() {
                self.network_service
                    .report_peer(*peer, rep::INACTIVE_SUBSTREAM);
                self.network_service
        ...
        match event {
            ...
            NotificationEvent::NotificationStreamOpened {
                ...
                match self.validate_connection(&peer, handshake, direction) {
                    Ok(handshake) => {
                        if self
                            .on_sync_peer_connected(peer, &handshake, direction)
                            .is_err()
                        {
                    ...
                    Err(wrong_genesis) => {
                        ...
                        if wrong_genesis {
                            self.peer_store_handle
                                .report_peer(peer, rep::GENESIS_MISMATCH);
                        }
    ...
    fn on_sync_peer_disconnected(&mut self, peer_id: PeerId) {
        ...
        if !self
            .default_peers_set_no_slot_connected_peers
            .remove(&peer_id)
            && info.inbound
        ...
        self.event_streams.retain(|stream| {
            stream
                .unbounded_send(SyncEvent::PeerDisconnected(peer_id))
                .is_ok()
        });
    }
    ...
    fn on_sync_peer_connected(
        ...
        if status.roles.is_full() {
            self.strategy
                .add_peer(peer_id, peer.info.best_hash, peer.info.best_number);
        }
        ...
        self.peer_store_handle
            .set_peer_role(&peer_id, status.roles.into());

        if self.default_peers_set_no_slot_peers.contains(&peer_id) {
            self.default_peers_set_no_slot_connected_peers
                .insert(peer_id);
        ...
        self.event_streams.retain(|stream| {
            stream
                .unbounded_send(SyncEvent::PeerConnected(peer_id))
                .is_ok()
        });
    ...
    fn send_state_request(&mut self, peer_id: PeerId, request: OpaqueStateRequest) {
        ...
        self.pending_responses
            .insert(peer_id, PeerRequest::State, rx.boxed());
    ...
    fn send_warp_proof_request(&mut self, peer_id: PeerId, request: WarpProofRequest<B>) {
        ...
        self.pending_responses
            .insert(peer_id, PeerRequest::WarpProof, rx.boxed());
        ...
        match response {
            Ok(Ok((resp, _))) => match request {
                ...
                PeerRequest::WarpProof => {
                    self.strategy
                        .on_warp_proof_response(&peer_id, EncodedProof(resp));
                }
            },
            Ok(Err(e)) => {
