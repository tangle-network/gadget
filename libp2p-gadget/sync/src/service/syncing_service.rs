...
impl<B: BlockT> SyncingService<B> {
    ...
    pub async fn num_queued_blocks(&self) -> Result<u32, oneshot::Canceled> {
        ...
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::NumQueuedBlocks(tx));
    ...
    pub async fn num_downloaded_blocks(&self) -> Result<usize, oneshot::Canceled> {
        ...
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::NumDownloadedBlocks(tx));
    ...
    pub async fn num_sync_requests(&self) -> Result<usize, oneshot::Canceled> {
        ...
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::NumSyncRequests(tx));
    ...
    pub fn on_block_finalized(&self, hash: B::Hash, header: B::Header) {
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::OnBlockFinalized(hash, header));
    }
...
impl<B: BlockT> JustificationSyncLink<B> for SyncingService<B> {
    ...
    fn request_justification(&self, hash: &B::Hash, number: NumberFor<B>) {
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::RequestJustification(*hash, number));
    }

    fn clear_justification_requests(&self) {
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::ClearJustificationRequests);
    }
}
...
impl<B: BlockT> Link<B> for SyncingService<B> {
    ...
    fn justification_imported(
        ...
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::JustificationImported(
                who, *hash, number, success,
            ));
    }
...
impl<B: BlockT> NetworkBlock<B::Hash, NumberFor<B>> for SyncingService<B> {
    fn announce_block(&self, hash: B::Hash, data: Option<Vec<u8>>) {
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::AnnounceBlock(hash, data));
    }

    fn new_best_block_imported(&self, hash: B::Hash, number: NumberFor<B>) {
        let _ = self
            .tx
            .unbounded_send(ToServiceCommand::NewBestBlockImported(hash, number));
    }
}
