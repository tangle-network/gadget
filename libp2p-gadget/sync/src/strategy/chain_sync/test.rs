...
fn do_ancestor_search_when_common_block_to_best_qeued_gap_is_to_big() {
    ...
    sync.on_block_data(&peer_id2, Some(peer2_req), response)
        .unwrap();
...
fn can_sync_huge_fork() {
    ...
    client
        .finalize_block(finalized_block.hash(), Some(just))
        .unwrap();
    ...
    sync.add_peer(
        peer_id1,
        common_block.hash(),
        *common_block.header().number(),
    );

    send_block_announce(
        fork_blocks.last().unwrap().header().clone(),
        peer_id1,
        &mut sync,
    );
    ...
    loop {
        ...
        request = if actions.is_empty() {
            ...
            assert_eq!(actions.len(), 1);
            match &actions[0] {
                ChainSyncAction::SendBlockRequest {
                    peer_id: _,
                    request,
                } => request.clone(),
...
fn syncs_fork_without_duplicate_requests() {
    ...
    while best_block_num < last_block_num {
        ...
        sync.on_block_data(&peer_id1, Some(request.clone()), response)
            .unwrap();
...
fn can_import_response_with_missing_blocks() {
    ...
    let blocks = (0..4)
        .map(|_| build_block(&mut client2, None, false))
        .collect::<Vec<_>>();
