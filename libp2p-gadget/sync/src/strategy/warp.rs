...
impl<Block: BlockT> WarpSyncParams<Block> {
    ...
    pub fn split(
        ...
        Option<oneshot::Receiver<<Block as BlockT>::Header>>,
    ) {
        match self {
    ...
    Client: HeaderBackend<B> + 'static,
{
    ...
    fn try_to_start_warp_sync(&mut self) {
        let Phase::WaitingForPeers { warp_sync_provider } = &self.phase else {
            return;
        };
    ...
    pub fn on_warp_proof_response(&mut self, peer_id: &PeerId, response: EncodedProof) {
        ...
        let Phase::WarpProof {
            ...
            self.actions.push(WarpSyncAction::DropPeer(BadPeer(
                *peer_id,
                rep::UNEXPECTED_RESPONSE,
            )));
            return;
        };
    ...
    fn target_block_request(&mut self) -> Option<(PeerId, BlockRequest<B>)> {
        let Phase::TargetBlock(target_header) = &self.phase else {
            return None;
        };
    ...
    pub fn status(&self) -> SyncStatus<B> {
        SyncStatus {
            state: match &self.phase {
                Phase::WaitingForPeers { .. } => SyncState::Downloading {
                    target: Zero::zero(),
                },
                Phase::WarpProof { .. } => SyncState::Downloading {
                    target: Zero::zero(),
                },
                Phase::PendingTargetBlock => SyncState::Downloading {
                    target: Zero::zero(),
                },
                Phase::TargetBlock(header) => SyncState::Downloading {
...
mod test {
    ...
    fn no_peer_is_scheduled_if_no_peers_connected() {
        ...
        assert!(warp_sync
            .schedule_next_peer(PeerState::DownloadingProofs, None)
            .is_none());
    }
    ...
    fn bad_warp_proof_response_drops_peer() {
        ...
        provider
            .expect_verify()
            .return_once(|_proof, _set_id, _authorities| {
        ...
        let actions = warp_sync.actions().collect::<Vec<_>>();
        assert_eq!(actions.len(), 1);
        let WarpSyncAction::SendWarpProofRequest {
    ...
    fn partial_warp_proof_doesnt_advance_phase() {
        ...
        provider
            .expect_verify()
            .return_once(|_proof, set_id, authorities| {
    ...
    fn complete_warp_proof_advances_phase() {
        ...
        provider
            .expect_verify()
            .return_once(move |_proof, set_id, authorities| {
    ...
    fn correct_target_block_response_sets_strategy_result() {
        ...
        assert!(warp_sync
            .on_block_response_inner(peer_id, request, response)
            .is_ok());
