...
pub async fn run_tangle_validator() -> Result<(), Box<dyn Error>> {
    ...
    let install = manager.run("binary_install".to_string(), &*"wget https://github.com/webb-tools/tangle/releases/download/v1.0.0/tangle-default-linux-amd64".to_string()).await?;
